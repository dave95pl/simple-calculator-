#tkinter - biblioteka w języku Python, która pomaga tworzyć graficzne interfejsy użytkownika. 
#Można go porównać do zestawu klocków, które pomagają budować różne rzeczy.

#Dzięki bibliotece Tkinter, programiści Pythona mogą tworzyć przyciski, pola tekstowe, okna dialogowe 
#i wiele innych elementów, które ułatwiają korzystanie z programów. 
import math 
from tkinter import *
...
# Ustawienie czcionki dla wszystkich widgetów w aplikacji
font_style = Font(family="Helvetica Bold", size=25)
root.option_add("*Font", font_style)

e = Entry(root, width=35, bg="azure4", fg="white", borderwidth=5)
...
def button_sqrt():
    number = e.get()
    e.delete(0, END)
    e.insert(0, math.sqrt(float(number)))

#określenie przycisków - rozmiary i działanie
...
button_sqrt = Button(root, text="sqrt", padx=28, pady=20, command=button_sqrt)


# umieszczanie przycisków w aplikacji (siatka)
